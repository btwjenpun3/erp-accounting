...
@section('js')
    <script>
        let table = $("#basic-1").DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('datatable.invoice.po-supplier') }}",
                type: 'GET',
                data: function(d) {
            ...
            columns: [{
                ...
                render: function(data) {
                    const total = data.reduce((sum, detail) => sum + detail.amount, 0);
        ...
        function format(d) {
            ...
            let detailRows = d.po.details.map(detail => `
            <tr>
                <td>${detail.material_code}</td>
                <td>${detail.material.material_type_code}</td>
                <td>${detail.material.uom_code}</td>
                <td>${detail.qty}</td>
                <td>${formatRupiah(detail.price_per_unit)}</td>
                <td>${formatRupiah(detail.amount)}</td>
            </tr>
            `).join('');

            return `
            <h4 class="text-center mb-3">Invoice Material Details</h4>
